...
# --- eye-in-hand camera / visibility settings ------------------------------
# The diagnostic camera looks STRAIGHT DOWN from above the gripper (fixed world -Z),
# so the top-down assumption in the VLM prompt always holds. Footprint half-width
# ~= tan(FOV/2) * (DIAGNOSE_HOVER_Z + CAM_STANDOFF). At FOV=60, hover=0.34 that is
# ~22cm each side -- plenty for any sabotage -- while keeping the cube a usable size.
CAM_RES = 512                   # diagnostic frame resolution
CAM_FOV_DEG = 60.0              # vertical field of view for the top-down camera
CAM_STANDOFF = 0.05             # camera height above the gripper (m)
DIAGNOSE_HOVER_Z = 0.34         # gripper height when the diagnostic frame is taken
CAM_CROSSHAIR = True            # draw a crosshair at the image centre = the gripper's target
CUBE_SCALE = 1.4                # enlarge the cube (~5cm -> ~7cm) so the top-down VLM can see it, still graspable
...
def capture_rgbd(robot, save_path=None, jitter=0.0):
    """Render the eye-in-hand view looking STRAIGHT DOWN from above the gripper
    (RGB + depth). The camera direction is fixed in world coordinates (-Z), so the
    top-down assumption the VLM prompt relies on always holds, regardless of the
    end-effector frame convention. A crosshair marks the image centre = the
    gripper's target."""
    pos, _ = ee_pose(robot)
    # Look straight down from well above the gripper's ground point. Camera height
    # above the TABLE (not the gripper) sets the footprint; a large standoff makes
    # the straight-down view numerically stable and free of any horizon tilt.
    cam_h = DIAGNOSE_HOVER_Z + CAM_STANDOFF        # camera height above the floor (m)
    eye = [pos[0] + jitter, pos[1] + jitter, cam_h]
    target = [pos[0], pos[1], -1.0]                # look far down the -Z axis (unambiguous)
    up = [0.0, 1.0, 0.0]                           # image +y = world +y
    view = p.computeViewMatrix(eye, target, up)
    proj = p.computeProjectionMatrixFOV(CAM_FOV_DEG, 1.0, 0.02, 3.0)
    _, _, rgba, _, _ = p.getCameraImage(CAM_RES, CAM_RES, view, proj,
                                        renderer=p.ER_TINY_RENDERER)
    if save_path:
        from PIL import Image, ImageDraw
        img = Image.fromarray(np.reshape(rgba, (CAM_RES, CAM_RES, 4))[:, :, :3].astype("uint8"))
        if CAM_CROSSHAIR:
            d = ImageDraw.Draw(img); c = CAM_RES // 2; g = max(6, CAM_RES // 40)
            col = (255, 60, 40)
            d.line([c, c - 3 * g, c, c - g], fill=col, width=2)
            d.line([c, c + g, c, c + 3 * g], fill=col, width=2)
            d.line([c - 3 * g, c, c - g, c], fill=col, width=2)
            d.line([c + g, c, c + 3 * g, c], fill=col, width=2)
            d.ellipse([c - g, c - g, c + g, c + g], outline=col, width=2)
        img.save(save_path)
    return save_path
...
class RealVLM:
    ...
    def diagnose(self, ftype, aim_xy, aim_yaw, obj_xy, obj_yaw, k, use_orientation,
        ...
        def _num(v, default=0.0):
            # Coerce the model's field to a float. A vision model often returns
            # null / "" / "N/A" for dx_cm when it cannot localise the object;
            # treat those as "no offset known" rather than crashing.
            if v is None:
                return None
        ...
        try:
            ...
            dx, dy = _num(d.get("dx_cm")), _num(d.get("dy_cm"))
            conf = _num(d.get("confidence"), 0.5)
            reasoning = str(d.get("reasoning", "")).strip()
            # If the model gave no usable correction, it is effectively saying
            # "I can't tell" -> return a zero move at LOW confidence so the
            # confidence gate re-queries or aborts instead of acting blindly.
            if dx is None or dy is None:
                print("    [RealVLM] no correction returned (dx/dy null) -> low-confidence no-op"
                      + (f"; VLM said: {reasoning[:80]}" if reasoning else ""))
                return {"dx_cm": 0.0, "dy_cm": 0.0, "dtheta_deg": 0.0,
                        "confidence": min(conf if conf is not None else 0.0, 0.20),
                        "reasoning": reasoning or "no correction returned",
                        "_frames": len(frames or []), "_latency_s": time.perf_counter() - t0}
            return {"dx_cm": dx, "dy_cm": dy,
                    "dtheta_deg": _num(d.get("dtheta_deg"), 0.0) or 0.0,
...
def run_episode(robot, obj_xy, obj_yaw, sabotage, vlm, *, closed, use_orientation=False,
    ...
    for attempt in range(1, MAX_RECOVERIES + 1):
        move(robot, [tgt_xy[0], tgt_xy[1], DIAGNOSE_HOVER_Z], tgt_yaw)  # hover to view
        ...
        if ftype == F_NO:
            ...
            move(robot, [tgt_xy[0], tgt_xy[1], DIAGNOSE_HOVER_Z], tgt_yaw)
            frames = _cap_frames(temporal_frames)
...
def probe_real_vlm(image_path=None):
    ...
    if image_path is None:
        ...
        move(robot, [0.50, 0.0, DIAGNOSE_HOVER_Z], 0.0)
