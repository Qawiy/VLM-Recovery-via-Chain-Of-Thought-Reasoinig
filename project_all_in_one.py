...
def _env_flag(name, default=False):
    """Read a boolean environment variable with common true spellings."""
    raw = os.getenv(name)
    if raw is None:
        return default
...
USE_REAL_VLM = _env_flag("USE_REAL_VLM", False)
VLM_MODEL = os.getenv("VLM_MODEL", "llama3.2-vision")
...
_BASE_INSTR = ("You are a top-down pick-and-place failure-diagnosis module with a "
               "downward eye-in-hand camera. Image centre = gripper target. Camera "
               "axes: +x image-right, +y image-up. Reason briefly, then output ONLY "
               'JSON: {"reasoning":"..","dx_cm":<n>,"dy_cm":<n>,"dtheta_deg":<n>,'
               '"confidence":<0-1>} = offset to ADD to the target to align on the object.')
PROMPTS = {
...
CLASSIFY_PROMPT = ('Classify this failed grasp. Reply ONLY JSON: '
                   '{"failure_type":"POSITIONAL_OFFSET|ORIENTATION|NO_OBJECT","confidence":<0-1>}')
...
class MockVLM:
    ...
    def classify(self, aim_xy, aim_yaw, obj_xy, obj_yaw, use_orientation, frames=None):
        """Classify the current failure type from simulated ground truth."""
    ...
    def diagnose(self, ftype, aim_xy, aim_yaw, obj_xy, obj_yaw, k, use_orientation,
        ...
        """Return a mock temporal correction and confidence for the failure."""
...
class RealVLM:
    """Live Llama 3.2 Vision via Ollama. Uses routed prompts and sends all temporal
    frames in one query. Coordinate args are ignored (it only sees the images)."""
    def __init__(self, model=VLM_MODEL):
        """Verify Ollama is available, store the model name, and run preflight."""
        if not _OLLAMA:
    ...
    def _preflight(self):
        """Confirm the configured Ollama model can answer a minimal JSON prompt."""
        try:
            ollama.chat(model=self.model, format="json",
                        messages=[{"role": "user",
                                   "content": 'Reply ONLY JSON: {"ok": true}'}],
                        options={"temperature": 0.0, "num_predict": 8})
        except Exception as e:
            msg = str(e)
            if "unknown model architecture" in msg and "mllama" in msg:
                raise RuntimeError(
                    "Ollama found the model, but its llama-server cannot load the "
                    "'mllama' architecture used by Llama 3.2 Vision. Update/reinstall "
                    "Ollama, then run: ollama pull llama3.2-vision. To run the "
                    "simulation without the real model, omit --real-vlm or set "
                    "USE_REAL_VLM=0."
                ) from e
            ...
            raise RuntimeError(f"Ollama real VLM preflight failed for '{self.model}': {e}") from e

    def _chat(self, system, images):
        """Send a system prompt and image list to the Ollama vision model."""
    ...
    def classify(self, aim_xy, aim_yaw, obj_xy, obj_yaw, use_orientation, frames=None):
        """Ask the real VLM to classify the failed grasp from image frames."""
        try:
            d = json.loads(self._chat(CLASSIFY_PROMPT, frames or []))
            return d.get("failure_type", F_POS), float(d.get("confidence", 0.5))
        except Exception:
            return F_POS, 0.5
    ...
    def diagnose(self, ftype, aim_xy, aim_yaw, obj_xy, obj_yaw, k, use_orientation,
        ...
        t0 = time.perf_counter()
        try:
            d = json.loads(self._chat(PROMPTS[ftype], frames or []))
...
def make_vlm():
    """The single switch: mock (anywhere) vs real Llama 3.2 Vision."""
...
def run_episode(robot, obj_xy, obj_yaw, sabotage, vlm, *, closed, use_orientation=False,
                ...
                mcs=None, tag=""):
    """Run one grasp attempt and optional closed-loop VLM recovery sequence."""
    def _cap_frames(k):
        """Capture k eye-in-hand frames for the real VLM backend."""
...
def _bar_chart(path, labels, rates, cis=None, title="", colors=("#c0563b", "#3b7dc0")):
    """Save a compact success-rate bar chart with optional confidence intervals."""
...
def experiment_base(n=30):
    """Run the base open-loop versus closed-loop recovery experiment."""
...
def experiment_advanced(ablation_n=25):
    """Run advanced qualitative cases and a sim-to-real noise ablation."""
...
def experiment_trials(n=120):
    """Run randomized position-and-yaw trials and save CSV/plots."""
...
def probe_real_vlm(image_path=None):
    """Probe the configured real VLM with one rendered or supplied image."""
    if not _OLLAMA:
    ...
    raw = ollama.chat(model=VLM_MODEL, format="json",
                      messages=[{"role": "system", "content": PROMPTS[F_POS]},
                                {"role": "user", "content": "Return the JSON.",
                                 "images": [image_path]}],
                      options={"temperature": 0.1})["message"]["content"]
    print("raw:", raw, f"\nlatency: {time.perf_counter()-t0:.1f}s")
...
def render_video(out_mp4=None):
    """Render a compact recovery video from the shared simulation primitives."""
    ...
    def grab(color=(150, 200, 255)):
        """Capture one video frame from the fixed scene camera."""
    ...
    def hstep(robot, n=1):
        """Advance physics and keep a held cube attached to the gripper."""
        for _ in range(n):
            p.stepSimulation()
            if held["on"]:
                e, _ = ee_pose(robot)
    ...
    def vmove(robot, xyz, yaw, steps=50, col=(150, 200, 255)):
        """Move the robot for the video and capture progress frames."""
        jt = p.calculateInverseKinematics(robot, PANDA_EE_LINK, list(xyz), yaw_to_quat(yaw))
        for j, t in zip(PANDA_ARM_JOINTS, jt[:7]):
            p.setJointMotorControl2(robot, j, p.POSITION_CONTROL, t, force=250)
    ...
    def vhold(robot, steps, col=(150, 200, 255)):
        """Pause the video while continuing to capture frames."""
        for i in range(steps):
            hstep(robot)
            if i % 3 == 0:
                grab(col)
    ...
    def vgrasp(robot, obj, axy, ayaw, oxy, oyaw, col):
        """Attempt one video grasp and attach the cube on success."""
        held["on"] = False
...
def main():
    """Parse command-line arguments and dispatch the selected experiment mode."""
    global _DOWN, USE_REAL_VLM
    ...
    ap.add_argument("--real-vlm", action="store_true",
                    help="use Ollama with VLM_MODEL, default llama3.2-vision")
    ap.add_argument("--mock-vlm", action="store_true",
                    help="force the deterministic mock backend")
    args = ap.parse_args()
    if args.real_vlm and args.mock_vlm:
        ap.error("--real-vlm and --mock-vlm are mutually exclusive")
    if args.real_vlm:
        USE_REAL_VLM = True
    if args.mock_vlm:
        USE_REAL_VLM = False
    ...
    print(f"VLM backend: {'RealVLM (' + VLM_MODEL + ')' if USE_REAL_VLM else 'MockVLM'}\n")
